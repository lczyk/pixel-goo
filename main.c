...
bool p_given = false;      // whether -p/--particles was explicitly passed
bool d_given = false;      // whether -d/--density was explicitly passed (p_given && d_given => error)
int whichMonitor = 0;
// int whichMonitor = 1;
...
int main(int argc, char **argv)
{
    ...
    // -p and -d are two ways to say the same thing (absolute count vs area density); reject both.
    if (p_given && d_given)
    {
        fprintf(stderr, "goo: --particles/-p and --density/-d are mutually exclusive\n");
        exit(EXIT_FAILURE);
    }
    ...
    // --fps-cap (default 60). ponytail: hardcoded codec, add --headless-args if you need to vary it.
    if (headless_path)
    {
