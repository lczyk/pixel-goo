...
int sim_width = 0, sim_height = 0; // simulation field size in sim px (logical/SIM_SCALE), set in window_setup
...
double headroomAttack = 0;       // --headroom-attack: EMA alpha when density RISES (close/darken; fast to avoid clipping)
double headroomRelease = 0;      // --headroom-release: EMA alpha when density FALLS (open/brighten; slower than attack)
...
static dropt_error parse_int(dropt_context *ctx, const dropt_option *opt, const char *arg, void *dest)
{
    ...
    if (end == arg || *end != '\0') return dropt_error_mismatch;
    ...
    if (r > (double)INT_MAX) return dropt_error_overflow;
    if (r < (double)INT_MIN) return dropt_error_underflow;
    *(int *)dest = (int)r;
...
static dropt_error parse_count(dropt_context *ctx, const dropt_option *opt, const char *arg, void *dest)
{
    ...
    if (e == dropt_error_none) p_given = true;
    return e;
}
...
static int config_handler(void *user, const char *section, const char *name, const char *value)
{
    ...
    if (!tok) { fprintf(stderr, "goo: out of memory parsing config\n"); exit(EXIT_FAILURE); }
...
static char *slurp_stdin(void)
{
    ...
    char *buf = malloc(cap);
    if (!buf) { fprintf(stderr, "goo: out of memory reading stdin\n"); exit(EXIT_FAILURE); }
    for (;;) {
        if (len + 1 >= cap) { cap *= 2; buf = realloc(buf, cap); if (!buf) { fprintf(stderr, "goo: out of memory reading stdin\n"); exit(EXIT_FAILURE); } }
...
static void parse_args(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-") == 0) want_stdin = true;
        // help wins over everything: honour -h/--help even if other args are bad.
        if (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0) want_help = true;
    }
    ...
    // nob.c). These come first so anything below overrides them silently.
    ...
    if (want_stdin) {
        ...
        free(buf);
        if (err < 0) { fprintf(stderr, "goo: cannot read config from stdin\n"); exit(EXIT_FAILURE); }
        if (err > 0) { fprintf(stderr, "goo: config parse error on stdin line %d\n", err); exit(EXIT_FAILURE); }
    }
    size_t stdin_end = cfg.count;

    // Layer 2: the cli, appended last so it wins (dropt keeps the last value).
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-") == 0) continue; // the stdin sentinel, not a flag
        tl_push(&cfg, argv[i]);
    }
    ...
    for (size_t i = stdin_start; i < stdin_end; i++) {
        char ckey[128];
        if (!token_flag_name(cfg.items[i], ckey, sizeof ckey)) continue;
...
int main(int argc, char **argv)
{
    ...
    // -p and -d are two ways to say the same thing (absolute count vs area density).
    // --density always has a value (default-params bakes one in), so they can't be
    // mutually exclusive anymore: -p simply wins when given (see the !p_given branch
    // below that derives P from density otherwise).
    ...
    // logical (screen) area now that window_setup has set width/height. logical area =
    ...
    while (!RGFW_window_shouldClose(window))
    {
        ...
        if (max_iterations > 0 && epoch_counter - warmup >= max_iterations) break;
...
void window_setup()
{
    ...
    sim_width  = (int)((double)logical_width  / SIM_SCALE + 0.5);
    sim_height = (int)((double)logical_height / SIM_SCALE + 0.5);
    mouse_scale = (logical_width > 0) ? (float)sim_width / (float)logical_width : 1.0f;
...
{
    (void)new_width; (void)new_height; // shape comes from the global sim/render dims now
    ...
    // shader_set_uniform_vec(&copyShader, "window_shape", 2, sim_shape);
